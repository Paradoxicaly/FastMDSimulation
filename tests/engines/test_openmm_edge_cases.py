...
class TestEdgeCases:
    ...
    def test_create_system_forcefield_unused_kwargs_retry(self):
        ...
        mock_ff.createSystem.side_effect = [
            ValueError("The argument 'bad_arg1' was specified to createSystem() but was never used."),
            ValueError("The argument 'bad_arg2' was specified to createSystem() but was never used."),
    ...
    def test_create_system_max_retries_prevention(self):
        ...
        def always_unused(*args, **kwargs):
            raise ValueError("The argument 'bad_arg' was specified to createSystem() but was never used.")
...
class TestSystemBuildingEdgeCases:
    ...
    def test_build_simulation_basic_functionality(self, tmp_path, sample_pdb_file):
        ...
        try:
            ...
            _build_simulation(Path(sample_pdb_file), {"forcefield": ["test.xml"]}, tmp_path)
            # If we get here, something unexpected happened
            pytest.fail("Expected _build_simulation to fail with missing forcefield files")
...
class TestAdvancedPlatformProperties:
    ...
    def test_new_simulation_with_platform_properties(self):
        ...
        with patch('openmm.app.Simulation') as mock_sim_class:
            with patch('fastmdsimulation.engines.openmm_engine._select_platform') as mock_select:
                mock_platform = Mock()
                ...
                result = _new_simulation(
                    mock_topology, mock_system, mock_integrator,
                    "CUDA", platform_props
                )
    ...
    def test_platform_property_logging(self):
        ...
        with patch('openmm.app.Simulation') as mock_sim_class:
            with patch('fastmdsimulation.engines.openmm_engine._select_platform') as mock_select:
                with patch('fastmdsimulation.engines.openmm_engine.logger') as mock_logger:
                    mock_platform = Mock()
                    ...
                    mock_platform.getPropertyDefaultValue.side_effect = get_property_side_effect
                    
                    _new_simulation(
                        mock_topology, mock_system, mock_integrator,
                        "TestPlatform", {"CustomProperty": "custom_value"}
                    )

...
class TestCreateSystemFunction:
    ...
    def test_create_system_retry_unused_kwargs(self):
        ...
        mock_ff.createSystem.side_effect = [
            ValueError("The argument 'invalid_arg' was specified to createSystem() but was never used."),
            mock_system
        ]
...
class TestBuildSimulation:
    ...
    def test_build_simulation_pdb(self, tmp_jobdir, sample_pdb_file):
        ...
        with patch('fastmdsimulation.engines.openmm_engine._build_simulation') as mock_build:
            mock_sim = Mock()
    ...
    def test_build_simulation_amber(self, tmp_jobdir):
        ...
        with patch('fastmdsimulation.engines.openmm_engine._build_from_amber') as mock_build:
            mock_sim = Mock()
            mock_build.return_value = mock_sim
            result = build_simulation_from_spec(spec, defaults, tmp_jobdir)
    ...
    def test_build_simulation_gromacs(self, tmp_jobdir):
        ...
        with patch('fastmdsimulation.engines.openmm_engine._build_from_gromacs') as mock_build:
            mock_sim = Mock()
            mock_build.return_value = mock_sim
            result = build_simulation_from_spec(spec, defaults, tmp_jobdir)
    ...
    def test_build_simulation_charmm(self, tmp_jobdir):
        ...
        with patch('fastmdsimulation.engines.openmm_engine._build_from_charmm') as mock_build:
            mock_sim = Mock()
            mock_build.return_value = mock_sim
            result = build_simulation_from_spec(spec, defaults, tmp_jobdir)

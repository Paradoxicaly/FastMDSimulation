...
def _maybe_barostat(system, ensemble: str, temperature_K: float, pressure_atm: float = 1.0):
...
def _constraints_from_str(name: Any):
    ...
    if isinstance(name, str):
        ...
        if key in ("none", "no", "off", "false"):
            return None
        if key == "hbonds":   return HBonds
        if key == "allbonds": return AllBonds
        if key == "hangles":  return HAngles
    return HBonds
...
def create_system(obj, *, topology=None, paramset=None, kwargs: Dict[str, Any] | None = None):
    """
    ...
    while True:
        try:
            ...
            if isinstance(obj, CharmmPsfFile):
                if paramset is None:
                    raise ValueError("create_system(): CharmmPsfFile requires 'paramset'.")
                return obj.createSystem(paramset, **public)

            raise TypeError(f"Unsupported object type for create_system(): {type(obj).__name__}")

        except ValueError as e:
            # OpenMM's ArgTracker message for unused kwargs
            m = re.search(r"The argument '([^']+)' was specified to createSystem\(\) but was never used\.", str(e))
            if not m:
                # A different error: bubble up
                raise
            bad = m.group(1)
            if bad in public:
                logger.warning(f"createSystem(): '{bad}' not used by this force field; dropping and retrying.")
                public.pop(bad, None)
                continue
...
def _new_simulation(topology, system, integrator, platform_name: str, platform_props: Dict[str, str] | None):
    ...
    defaults = []
    for k in ("CudaDeviceIndex", "CudaPrecision", "OpenCLDeviceIndex", "OpenCLPlatformIndex"):
        try:
    ...
    if props:
        for k, v in props.items():
            defaults.append(f"{k}={v}")
    logger.info("Platform: " + plat.getName() + (f" ({', '.join(defaults)})" if defaults else ""))
    return sim
...
def _make_integrator(defaults: Dict[str, Any]):
    ...
    temperature_K = float(spec.get("temperature_K", defaults.get("temperature_K", 300.0)))
    ...
    if name == "langevin":
        return LangevinIntegrator(temperature_K*unit.kelvin,
        ...
        return BrownianIntegrator(temperature_K*unit.kelvin,
                                  friction_ps/unit.picoseconds,
                                  timestep_fs*unit.femtoseconds)
    elif name == "verlet":
        return VerletIntegrator(timestep_fs*unit.femtoseconds)
    elif name == "variable_langevin":
        return VariableLangevinIntegrator(temperature_K*unit.kelvin,
                                          friction_ps/unit.picoseconds,
                                          errtol)
...
def _build_simulation(pdb_path: Path, defaults: Dict[str, Any], run_dir: Path):
    ...
    if "constraints" not in cs_kwargs:
        cs_kwargs["constraints"] = _constraints_from_str(defaults.get("constraints", "HBonds"))
    ...
    sim = _new_simulation(modeller.topology, system, integrator, platform_name, platform_props)
...
def _build_from_amber(spec: Dict[str, Any], defaults: Dict[str, Any], run_dir: Path):
    ...
    integrator = _make_integrator(defaults)
    sim = _new_simulation(prmtop.topology, system, integrator, defaults.get("platform", "auto"), defaults.get("platform_properties"))
...
def _build_from_gromacs(spec: Dict[str, Any], defaults: Dict[str, Any], run_dir: Path):
    ...
    integrator = _make_integrator(defaults)
    sim = _new_simulation(top.topology, system, integrator, defaults.get("platform", "auto"), defaults.get("platform_properties"))
...
def _build_from_charmm(spec: Dict[str, Any], defaults: Dict[str, Any], run_dir: Path):
    ...
    for key in ("prm", "rtf", "str"):
        if key in spec:
            ...
            params.extend([str(x) for x in v] if isinstance(v, (list, tuple)) else [str(v)])
    if not params:
        raise ValueError("CHARMM spec requires parameter files via 'params' or 'prm/rtf/str'.")
    ...
    integrator = _make_integrator(defaults)
    sim = _new_simulation(psf.topology, system, integrator, defaults.get("platform", "auto"), defaults.get("platform_properties"))
...
# Public dispatcher
# ------------------------------------------------------------
def build_simulation_from_spec(spec: Dict[str, Any], defaults: Dict[str, Any], run_dir: Path):
    stype = (spec.get("type") or "").lower()
    if not stype:
        if "pdb" in spec: stype = "pdb"
        elif "prmtop" in spec and ("inpcrd" in spec or "rst7" in spec): stype = "amber"
        elif "top" in spec and "gro" in spec: stype = "gromacs"
        elif "psf" in spec: stype = "charmm"
        else: raise ValueError(f"Cannot infer simulation type from spec: {spec}")

    run_dir.mkdir(parents=True, exist_ok=True)

    if stype == "pdb":     return _build_simulation(Path(spec["pdb"]), defaults, run_dir)
    if stype == "amber":   return _build_from_amber(spec, defaults, run_dir)
    if stype == "gromacs": return _build_from_gromacs(spec, defaults, run_dir)
    if stype == "charmm":  return _build_from_charmm(spec, defaults, run_dir)
...
def run_stage(sim, stage: Dict[str, Any], stage_dir: Path, defaults: Dict[str, Any]):
    ...
    report_interval = int(stage.get("report_interval", defaults.get("report_interval", 1000)))
    checkpoint_interval = int(stage.get("checkpoint_interval", defaults.get("checkpoint_interval", 10000)))
    ...
    sim.reporters.append(CheckpointReporter(str(stage_dir/'state.chk'), checkpoint_interval))

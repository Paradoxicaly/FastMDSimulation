...
def _read_log_style_from_yaml(yaml_path: str | Path) -> str | None:
    try:
        ...
        sty = (((data.get("defaults") or {}).get("log_style")) if isinstance(data, dict) else None)
...
def _resolve_plan_from_pdb(pdb: str, outdir: str, config: str | None):
    ...
    runs = [{
        ...
        "run_dir": str(tmp / f'auto_T{yml_like["defaults"].get("temperature_K", 300)}'),
        "stages": [{
            "name": st["name"],
...
def main():
    ...
    p_sim.add_argument("--atoms", type=str, default=None,
                       help='Atom selection (e.g., "protein")')
    p_sim.add_argument("--slides", choices=["True", "False"], default="True",
    ...
    if args.cmd == "simulate":
        ...
        if system.lower().endswith((".yml", ".yaml")):
            ...
            if args.dry_run:
                ...
                print(
                    ...
                    + (f' (slides={args.slides}, frames={args.frames}, atoms={args.atoms})' if args.analyze else "")
                )
                for r in plan["runs"]:
                    print(f'- Run: {r["system_id"]} @ {r["temperature_K"]} K -> {r["run_dir"]}')
                    for s in r["stages"]:
                        print(f'    · {s["name"]}: {s["steps"]} steps (~{s["approx_ps"]} ps)')
                    if args.analyze:
                        prod = Path(r["run_dir"]) / "production"
                        cmd = build_analyze_cmd(prod / "traj.dcd", prod / "topology.pdb",
            ...
            if args.dry_run:
                ...
                for r in plan["runs"]:
                    ...
                    if args.analyze:
                        ...
                        print("    → fastmda command:", " ".join(map(str, cmd)))
                return
            project_dir = simulate_from_pdb(system, outdir=args.output, config=args.config)
        ...
        if args.analyze:
            ok = analyze_with_bridge(project_dir, slides=(args.slides == "True"),
                                     frames=args.frames, atoms=args.atoms)
            if not ok:
                print("Analysis skipped or failed; install FastMDAnalysis or adjust flags.")